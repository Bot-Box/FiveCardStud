"""
Module for playing games of Five Card Stud using PokerTextProtocol
...
class GtpConnection():
    ...
    def genmove_cmd(self, args):
        ...
        try:
            ...
            if move is None:
                self.respond("resign")
                return
            winner, pos = self.solve_cmd_2()
            if pos != "" and winner != "":
                self.play_cmd_2((board_color, pos))
                self.respond(pos)
            else:
    ...
    def time_limit_cmd(self, args):
        if int(args[0]) > 100 or int(args[0]) < 1:
            self.respond("Error: Time limit value not allowed")
        else:
    ...
    def undoMove(self, move):
        ...
        self.board.to_play = GoBoardUtil.opponent(self.board.to_play)
    ...
    def solve_cmd(self, args=""):
        ...
        try:
            ...
            winner = self.board.to_play
            win = self.resultForBlack(self.board)
            signal.alarm(0)
            if win:
                if winner == BLACK:
                    self.respond("b " + WIN_POS)
                else:
                    self.respond("b")
            else:
                if winner == BLACK:
                    ...
                    self.respond("w " + WIN_POS)
        except ValueError as e:
            # self.board = temp
            self.respond("unknown")

    def solve_cmd_2(self, args=""):
        global WIN_POS
        signal.signal(signal.SIGALRM, self.handler)
        try:
            # temp = self.board.copy()
